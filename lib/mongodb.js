...
const SMARTCHOICE_DB_NAME = 'smartchoice';
...
const cache = global.__smartChoiceMongo ?? {
  client: null,
  db: null,
  dbName: SMARTCHOICE_DB_NAME,
  connectPromise: null,
  cooldownUntil: 0,
  indexesReady: false,
};
global.__smartChoiceMongo = cache;

const legacyMongoGlobalKey = '__shopping' + 'SmartMongo';
...
async function ensureIndexes(db, { apiCacheCollection, priceHistoryCollection, barcodeScanCollection }) {
  ...
  await db.collection(priceHistoryCollection).createIndex(
    { productId: 1, bucketMonth: -1 },
    { name: 'price_history_product_month' }
  );
  await db.collection(priceHistoryCollection).createIndex(
    { barcode: 1, bucketMonth: -1 },
    { name: 'price_history_barcode_month' }
  );
  if (barcodeScanCollection) {
    await db.collection(barcodeScanCollection).createIndex(
      { expiresAt: 1 },
      { expireAfterSeconds: 0, name: 'barcode_scans_ttl' }
    );
  }
...
function resetConnection() {
  ...
  cache.client = null;
  cache.db = null;
  cache.dbName = SMARTCHOICE_DB_NAME;
...
async function connectMongo(indexCollections) {
  ...
  if (cache.db && cache.dbName === SMARTCHOICE_DB_NAME) return cache.db;
  if (cache.db && cache.dbName !== SMARTCHOICE_DB_NAME) resetConnection();
